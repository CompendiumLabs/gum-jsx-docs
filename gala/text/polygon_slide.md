...
The overall composition is handled with layout elements rather than manual positioning. A [Slide](/docs/Slide) provides the title and sets the em with `em={0.045}`, and a [TextGrid](/docs/TextGrid) lays out the six cards in three columns. Each card is a [Frame](/docs/Box) around a [TextFigure](/docs/TextFigure): the polygon is three and a half ems tall and its label is a caption at the text size, so every card sizes from the same em and nothing needs a `stack-size`.
