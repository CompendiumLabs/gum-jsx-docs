...
This is the bare decoration. [Latex](/docs/Latex) places it over or under a body, stretched to the body's width, and [HorizBrace](/docs/HorizBrace) does the same for a brace with a label. On its own it is a math item of the given width and its natural height that can be dropped into a [MathText](/docs/MathText), which is handy for a long arrow between two expressions. It is classed as a relation, so a row spaces it the way `\xrightarrow` and friends are spaced in LaTeX; pass `klass` to space it as something else.
