...
<Svg width={px(600)} color={slate}>
  <Box width={1} padding={px(16)}>
    <VStack width={1} gap={px(14)}>
      <Text font-size={px(24)} font-weight={bold}>Choose the point that meets the position</Text>
      <Group aspect={3}>
        {['start', 'center', 'end'].map((anchor, index) =>
          <Group x={index / 3} width={1 / 3}>
            ...
            <Text x={0.5} y={0.92} anchor="center" font-size={px(12)} color={slate}>x = 0.5, y = 0.5</Text>
          </Group>
        )}
      </Group>
    </VStack>
