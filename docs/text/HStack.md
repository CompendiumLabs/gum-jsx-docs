...
axis. See [Stack](Stack.md) for all props and allocation rules.
...
- align="baseline" aligns the first text baselines; a child without a baseline
  uses its bottom edge. The row includes both ascent and descent extents.
- justify controls horizontal positioning or distributed spacing.

Stretch allocates the row's selected height to its children; it can override
