...
figures (`gala/`), and the loaders that index them into pages. Pure content plus a small
node-only reader — no dependency on `@gum-jsx/core` or any other sibling, so nothing here
evaluates or renders. The examples are `.jsx` source; the packages that consume them do the
rendering:

- `gum-jsx` (`../gum-jsx`): renders every example in strict mode as its test suite
  (`scripts/test.ts`), and builds the Claude skill out of the text and code here
  (`scripts/skill.ts`).
