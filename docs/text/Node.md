...
Without an `em`, the node is a [Frame](/docs/Frame) of the given `ysize` with the label fit into it, so the text size follows from the box and the number of lines, and `padding` is in em of the label. In both cases, `rounded` uses stroke units and does not change with the node's size.
