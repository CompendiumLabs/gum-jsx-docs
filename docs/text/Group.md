...
- `em` — coordinate units per em: a child with metrics (a [Text](/docs/Text), a formula, a text container) placed by `pos` with no size of its own is made its em height times this tall, so labels dropped into the frame share one text size; applies to direct children only, a nested group's coordinates being its own. Without one, a bare **Group** laid out by its container (the content of a [Box](/docs/Box), a [Slide](/docs/Slide) or the canvas, rather than placed by `rect`) takes the ambient em, so such a label comes out at the surrounding text size, as it would in a **Box**
